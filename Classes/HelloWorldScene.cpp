...
/*
	Size visibleSize = Director::getInstance()->getVisibleSize();
		...
		log("%f,%s,%f",origin.x,"origin",origin.y);
		log("%f,%s,%f",visibleSize.width,"visibleSize",visibleSize.height);
...
void HelloWorld::loadResource(float dt) {
	...
	auto closeItem = MenuItemImage::create("close00.png", "close01.png",
			CC_CALLBACK_1(HelloWorld::menuCloseCallback, this));

	closeItem->setPosition(
			Vec2(
					origin.x + visibleSize.width
							- closeItem->getContentSize().width * 2 / 3,
					origin.y + visibleSize.height
							- closeItem->getContentSize().height * 2 / 3));

	auto beginItem = MenuItemImage::create("begin01.png", "begin02.png",
			CC_CALLBACK_1(HelloWorld::menuGotoGameCallback, this));
	beginItem->setPosition(
			Vec2(origin.x + visibleSize.width / 2,
					origin.y + beginItem->getContentSize().height));
	...
	sprite->setPosition(
			Vec2(origin.x + visibleSize.width / 2,
					origin.y + visibleSize.height
							- sprite->getContentSize().height * 3 / 2));
	this->addChild(sprite, 1);
	/*
	SimpleAudioEngine::sharedEngine()->preloadBackgroundMusic("snd/snd_bg.ogg");
	SimpleAudioEngine::sharedEngine()->playBackgroundMusic("snd/snd_bg.ogg", true);
	log("HelloWold playBkMusic");
	*/
...
void HelloWorld::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event) {
	if (keyCode == EventKeyboard::KeyCode::KEY_BACK
			|| keyCode == EventKeyboard::KeyCode::KEY_HOME
			|| keyCode == EventKeyboard::KeyCode::KEY_KP_HOME
		) {
...
void HelloWorld::onEnter() {
	...
	if (m_key_lstnr == nullptr) {
		...
		m_key_lstnr->onKeyPressed = CC_CALLBACK_2(HelloWorld::onKeyPressed,
				this);
		m_key_lstnr->onKeyReleased = CC_CALLBACK_2(HelloWorld::onKeyReleased,
				this);
